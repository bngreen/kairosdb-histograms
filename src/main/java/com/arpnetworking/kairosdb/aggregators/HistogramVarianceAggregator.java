...
/**
 * Aggregator that computes the variance value of histograms.
 *
 * @author Bruno Green (bruno dot green at gmail dot com)
 */
@AggregatorName(
        name = "hvar",
        description = "Computes the variance value of the histograms.",
        properties = {
                @AggregatorProperty(name = "sampling", type = "duration"),
                @AggregatorProperty(name = "align_start_time", type = "boolean")
        }
)
public class HistogramVarianceAggregator extends RangeAggregator {
    /**
     * Public constructor.
     *
     ...
     */
    @Inject
    public HistogramVarianceAggregator(final DoubleDataPointFactory dataPointFactory) throws KairosDBException {
        _dataPointFactory = dataPointFactory;
    }

    @Override
    protected RangeSubAggregator getSubAggregator() {
        return new HistogramVarianceDataPointAggregator();
    }
    ...
    /**
     * Data point aggregator that computes the variance value of histograms.
     *
     * @author Bruno Green (bruno dot green at gmail dot com)
     */
    protected class HistogramVarianceDataPointAggregator implements RangeSubAggregator {

        /**
         *
         * @param count the number of samples
         * @param mean the mean of the samples
         * @param m2 squared distance from the mean
         * @return the final value for the aggregator (default implementation returns the variance)
         */
        ...
        public Iterable<DataPoint> getNextDataPoints(final long returnTime, final Iterator<DataPoint> dataPointRange) {
            ...
            return Collections.singletonList(_dataPointFactory.createDataPoint(returnTime, computeFinalValue(count, mean, m2)));
        }
    }
}
